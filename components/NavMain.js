...
const DropDownItem = ({ data }) => {
  ...
  return (
    <li
      className={cx(
        "m-0 hover:text-substrateGreen hover:underline dark:text-white font-medium cursor-pointer",
        {
        ...
        <div ref={ref}>
          <div
            className={cx(itemClass, "pr-24", {
              "bg-substrateGreen-light dark:bg-green-700 underline":
                isComponentVisible,
            })}
          ...
          {isComponentVisible && (
            <ChildMenu
              slugPrefix={data.menuItem.url + data.subMenuItem.url}
      ...
      >
        <span
          className={cx("font-medium", {
            "text-substrateGreen":
              (isComponentVisible && header === "default") ||
              (isComponentVisible && header === "home" && isScrolled),
            "text-white":
              isComponentVisible && header === "home" && !isScrolled,
            "group-hover:text-substrateGreen":
        ...
        <Icon
          name="arrow-dropdown"
          className={cx(
            "inline-block align-middle xl:ml-2 fill-current dark:text-white",
            {
